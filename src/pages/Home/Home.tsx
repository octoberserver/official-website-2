  ...
  return (
    <>
      <Header background={`url(${bannerBg.src})`} className="h-[35rem]">
        ...
        <button className="pt-1 px-4 pb-1.5 flex justify-center text-white text-center items-center bg-[#DF0000] rounded-[1rem] md:rounded-[1.25rem]">
          <span className="text-center text-3xl">開始遊玩!</span>
          <Image src={playIcon} className="h-7 w-8 md:h-10 md:w-11 pl-1 no-drag_c" alt="附魔鑽石鎬"/>
        </button>
      </Header>
...
function HowToJoinModal() {
  <dialog how-to-join-modal>
    <div how-to-join-modal-inner className="relative p-6 grid gap-5 bg-2D2D2D_c text-center rounded-3xl text-[1.75rem]">
      <a className="absolute right-2 top-2 grid place-content-center cursor-pointer" href="javascript:document.querySelector('[how-to-join-modal]').close();">
      ...
      <div className="py-3 px-5 rounded-2xl select-none bg-393939_c">
        <h3>伺服器位址：</h3>
      ...
      <div className="py-3 px-5 rounded-2xl bg-393939_c">
        <p>
          我們是模組服<br/>
          <span className="text-2xl">需要使用特殊啟動器才能加入!</span>
        </p>
        <a href="downloads#main-pack-launcher" target="_blank">
          <button className="mt-1 py-1 px-3 rounded-xl bg-DF0000_c select-none text-2xl">下載啟動器</button>
        </a>
      </div>
    </div>
  </dialog>
